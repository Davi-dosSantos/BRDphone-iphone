...
class CoreContext: ObservableObject {
	...
	func initialiseCore() throws {
		...
		coreQueue.async {
			...
			Log.info(
				"Checking if linphonerc file exists already. If not, creating one as a copy of linphonerc-default"
			)
			if let rcDir = FileManager.default.containerURL(
				forSecurityApplicationGroupIdentifier: Config.appGroupName)?
				...
				if !FileManager.default.fileExists(atPath: rcFileUrl.path) {
					do {
						try FileManager.default.createDirectory(
							at: rcDir, withIntermediateDirectories: true)
						if let pathToDefaultConfig = Bundle.main.path(
							forResource: "linphonerc-default", ofType: nil)
						{
							try FileManager.default.copyItem(
								at: URL(fileURLWithPath: pathToDefaultConfig), to: rcFileUrl)
							Log.info("Successfully copied linphonerc-default configuration")
						}
					} catch let error {
						Log.error(
							"Failed to copy default linphonerc file: \(error.localizedDescription)")
					}
				} else {
					Log.info(
						"Found existing linphonerc file, skip copying of linphonerc-default configuration"
					)
				}
			}

			self.mCore = try? Factory.Instance.createSharedCoreWithConfig(
				config: Config.get(), systemContext: Unmanaged.passUnretained(coreQueue).toOpaque(),
				appGroupId: Config.appGroupName, mainCore: true)
			...
			let userAgent =
				"LinphoneiOS/\(version ?? "6.0.0") (\(UIDevice.current.localizedModel.replacingOccurrences(of: "'", with: ""))) LinphoneSDK"
			...
			// Migration
			self.mCore.config!.setBool(
				section: "sip", key: "auto_answer_replacing_calls", value: false)
			self.mCore.config!.setBool(section: "sip", key: "deliver_imdn", value: false)
			self.mCore.config!.setString(
				section: "misc", key: "log_collection_upload_server_url",
				value: "https://files.linphone.org:443/http-file-transfer-server/hft.php")
			self.mCore.config!.setString(
				section: "misc", key: "file_transfer_server_url",
				value: "https://files.linphone.org:443/http-file-transfer-server/hft.php")
			self.mCore.config!.setString(
				section: "misc", key: "version_check_url_root",
				value: "https://download.linphone.org/releases")
			...
			let shortcutsCount = self.mCore.config!.getInt(
				section: "ui", key: "shortcut_count", defaultValue: 0)
			if shortcutsCount > 0 {
				var shortcuts: [ShortcutModel] = []
				for i in 0...shortcutsCount {
					let shortcutSection = "shortcut_\(i)"
					let link = self.mCore.config!.getString(
						section: shortcutSection, key: "link", defaultString: "")
					let linkUrl = URL(string: link)
					let name = self.mCore.config!.getString(
						section: shortcutSection, key: "name", defaultString: "")
					let iconLink = self.mCore.config!.getString(
						section: shortcutSection, key: "icon", defaultString: "")
					let iconLinkUrl = URL(string: iconLink)

					if linkUrl == nil {
						Log.error(
							"Could not add shortcut #\(i) pointing to \(name) because the link URL '\(link)' is invalid"
						)
						continue
					}
					if iconLinkUrl == nil {
						Log.error(
							"Could not add shortcut #\(i) pointing to \(name) because the icon link URL '\(iconLink)' is invalid"
						)
						continue
					}
					shortcuts.append(
						ShortcutModel(linkUrl: linkUrl!, name: name, iconLinkUrl: iconLinkUrl!))
				}
			...
			self.mCoreDelegate = CoreDelegateStub(
				onGlobalStateChanged: { (core: Core, state: GlobalState, _: String) in
					if state == GlobalState.On {
						...
						for account in core.accountList {
							if account.params?.pushNotificationConfig?.provider
								!= ("apns" + pushEnvironment)
							{
								let newParams = account.params?.clone()

								Log.info(
									"Account \(String(describing: newParams?.identityAddress?.asStringUriOnly())) - updating apple push provider from \(String(describing: newParams?.pushNotificationConfig?.provider)) to apns\(pushEnvironment)"
								)
								newParams?.pushNotificationConfig?.provider =
									"apns" + pushEnvironment
						...
						if CorePreferences.defaultDomain == "" {
							CorePreferences.defaultDomain =
								core.defaultAccount?.params?.domain ?? ""
						}
				...
				onCallStateChanged: {
					(core: Core, call: Call, cstate: Call.State, message: String) in
					Log.info(
						"[CoreContext] onCallStateChanged - state: \(cstate), callId: \(call.callLog?.callId ?? "unknown")"
					)
					TelecomManager.shared.onCallStateChanged(
						core: core, call: call, state: cstate, message: message)

					if core.calls.isEmpty {
						DispatchQueue.main.asyncAfter(deadline: .now() + 2) {
							if UIApplication.shared.applicationState == .background {
								Log.info(
									"[CoreContext] core is currently in background with no calls, triggering onEnterBackground procedure"
								)
				...
				onAuthenticationRequested: { (core: Core, authInfo: AuthInfo, method: AuthMethod) in
					if method == .Bearer {
						if let server = authInfo.authorizationServer, !server.isEmpty {
							Log.info(
								"Authentication requested method is Bearer, starting Single Sign On activity with server URL \(server) and username \(authInfo.username ?? "")"
							)
							self.bearerAuthInfoPendingPasswordUpdate = authInfo
							SingleSignOnManager.shared.setUp(
								ssoUrl: server, user: authInfo.username ?? "")
						}
					}

					if method == .HttpDigest {
						guard let username = authInfo.username, let domain = authInfo.domain,
							let realm = authInfo.realm
						else {
							Log.error(
								"Authentication requested but either username [\(String(describing: authInfo.username))], domain [\(String(describing: authInfo.domain))] or server [\(String(describing: authInfo.authorizationServer))] is nil or empty!"
							)
							return
						}

						guard
							let accountFound = core.accountList.first(where: {
								$0.params?.identityAddress?.username == authInfo.username
									&& $0.params?.identityAddress?.domain == authInfo.domain
							})
						else {
							Log.info(
								"[CoreContext] Failed to find account matching auth info, aborting auth dialog"
							)
							return
						}

						let identity =
							"\(authInfo.username ?? "username")@\(authInfo.domain ?? "domain")"
						Log.info(
							"[CoreContext] Authentication requested method is HttpDigest, showing dialog asking user for password for identity [\(identity)]"
						)
				...
				onTransferStateChanged: { (_: Core, transferred: Call, callState: Call.State) in
					Log.info(
						"[CoreContext] Transferred call \(transferred.remoteAddress!.asStringUriOnly()) state changed \(callState)"
					)
					DispatchQueue.main.async {
						if callState == Call.State.Connected {
							ToastViewModel.shared.toastMessage =
								"Success_toast_call_transfer_successful"
							ToastViewModel.shared.displayToast = true
						} else if callState == Call.State.OutgoingProgress {
							ToastViewModel.shared.toastMessage =
								"Success_toast_call_transfer_in_progress"
							ToastViewModel.shared.displayToast = true
						...
						if status == ConfiguringState.Successful {
							...
							for account in self.mCore.accountList {
								accountModels.append(
									AccountModel(account: account, core: self.mCore))
							}
							self.accounts = accountModels
				...
				onLogCollectionUploadStateChanged: {
					(_: Core, _: Core.LogCollectionUploadState, info: String) in
					if info.starts(with: "https") {
						DispatchQueue.main.async {
							UIPasteboard.general.setValue(
								info, forPasteboardType: UTType.plainText.identifier)
				...
				onAccountRegistrationStateChanged: {
					...
					Log.info(
						"New registration state is \(state) for user id "
					...
					case .Ok:
						DispatchQueue.main.async {
							NotificationCenter.default.post(
								name: NSNotification.Name("CoreStarted"), object: nil)
						}
						// BRD: Removido fetchContacts() do startup para não atrasar chamadas VoIP
						// ContactsManager.shared.fetchContacts()

						if let defaultAccountParams = self.mCore.defaultAccount?.params,
							defaultAccountParams.publishEnabled == false
						{
						...
						if self.mCore.consolidatedPresence != ConsolidatedPresence.Online {
							self.updatePresence(
								core: self.mCore, presence: ConsolidatedPresence.Online)
						}
					case .Cleared:
						Log.info(
							"[CoreContext][onAccountRegistrationStateChanged] Account \(account.displayName()) registration was cleared."
						)
					case .Failed:
						Log.error(
							"[CoreContext][onAccountRegistrationStateChanged] Account \(account.displayName()) registration failed."
						)
					default:
						break
					}
					...
					TelecomManager.shared.onAccountRegistrationStateChanged(
						core: core, account: account, state: state, message: message)
				...
				onDefaultAccountChanged: { (_: Core, account: Account?) in
					if let account = account, self.mCore.globalState == GlobalState.On {
						Log.info(
							"[CoreContext][onDefaultAccountChanged] Default account set to: \(account.displayName())"
						)
						DispatchQueue.main.async {
							...
							NotificationCenter.default.post(
								name: NSNotification.Name("DefaultAccountChanged"), object: nil)
						}
	...
	func onEnterBackground() {
		coreQueue.async {
			Log.info(
				"[onEnterForegroundOrBackground] Entering background, un-PUBLISHING presence info")
			...
			if self.mCore.currentCall == nil && self.mCore.globalState == .On {
				...
				Log.info(
					"[onEnterForegroundOrBackground] Skipped stop: core not fully On or active call in progress"
				)
			}
		}
	}
	...
	func forceRemotePushToMatchVoipPushSettings(account: Account) {
		if let params = account.params,
			params.pushNotificationAllowed && !params.remotePushNotificationAllowed
		{
			Log.warn(
				"account \(account.displayName()): VOIP and REMOTE push setting mismatch, force \(params.pushNotificationAllowed ? "enabling" : "disabling") of REMOTE Push"
			)
			...
			if directory != nil {
				do {
					if FileManager.default.fileExists(
						atPath: directory!.appendingPathComponent("linphone.db").path)
					{
						try FileManager.default.removeItem(
							at: directory!.appendingPathComponent("linphone.db"))
					}
					try FileManager.default.copyItem(
						at: rcFileUrl, to: directory!.appendingPathComponent("linphone.db"))
				} catch {
					print("Error: ", error)
				}
			}
