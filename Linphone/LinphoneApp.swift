	...
	PKPushRegistryDelegate
{
	...
	func application(
		...
		Log.info(
			"BRD_APNS: Firebase usará este token para gerar FCM token e intermediar push via APNS")
		...
		if let callType = userInfo["type"] as? String, callType == "call" {
			...
			if let creationToken = creationToken {
				NotificationCenter.default.post(
					name: accountTokenNotification, object: nil, userInfo: ["token": creationToken])
			}
			completionHandler(.newData)
		}
	}
	...
	private func setupPushKit() {
		...
		Log.info(
			"[DEBUG] PKPushRegistry.delegate set to AppDelegate: \(String(describing: pushRegistry?.delegate))"
		)
	...
	func pushRegistry(
		...
		Task {
			if let coreContext = coreContext as CoreContext?,
				let account = coreContext.mCore?.defaultAccount
					?? coreContext.mCore?.accountList.first,
				let identity = account.params?.identityAddress?.asStringUriOnly()
			{
				await MiddlewareServices.registerAPNSToken(
					userID_Domain: identity, voipToken: token)
				Log.info("BRD_PUSHKIT: Token VoIP enviado para API")
			} else {
				Log.warn(
					"BRD_PUSHKIT: Conta ainda não disponível, token VoIP será enviado quando conta estiver pronta"
				)
	...
	func pushRegistry(
		...
		Log.info("[DEBUG] Entrou em pushRegistry(_:didReceiveIncomingPushWith:for:completion:)")
		...
		let fromUser =
			payload.dictionaryPayload["caller_id"] as? String
			?? payload.dictionaryPayload["from"] as? String
			?? "sip:unknown@unknown"
		let displayName =
			payload.dictionaryPayload["display_name"] as? String
			?? payload.dictionaryPayload["display-name"] as? String
			?? "Chamada recebida"
		let callUUID =
			payload.dictionaryPayload["call_uuid"] as? String
			?? UUID().uuidString  // Gera UUID temporário se não houver
		...
		Log.info("BRD_PUSHKIT: call-uuid: \(callUUID)")
		...
		let uuid = UUID()
		TelecomManager.shared.currentCallKitUUID = uuid  // Salva o UUID gerado para correlação
		...
		if TelecomManager.callKitEnabled() {
			...
			// Usa pushUUID como identificador temporário no rastreamento inicial
			let tempId = TelecomManager.shared.pushUUID ?? "unknown-push-uuid"
			let callInfo = CallInfo.newIncomingCallInfo(callId: tempId)
			providerDelegate.callInfos[uuid] = callInfo
			providerDelegate.uuids[tempId] = uuid
			...
			providerDelegate.provider.reportNewIncomingCall(with: uuid, update: update) { error in
				if let error = error {
					Log.error(
						"BRD_PUSHKIT:   Erro ao reportar chamada ao CallKit: \(error.localizedDescription)"
					)
			...
			Log.error(
				"BRD_PUSHKIT: CallKit DESABILITADO - chamadas com tela bloqueada NÃO FUNCIONARÃO!")
	...
	func userNotificationCenter(
		...
		if let callId = userInfo["CallId"] as? String,
			let peerAddr = userInfo["peer_addr"] as? String,
			let localAddr = userInfo["local_addr"] as? String
		{
				...
				if let coreContext = coreContext {
					coreContext.doOnCoreQueue { core in
						...
						if let peerAddr = try? Factory.Instance.createAddress(addr: strPeerAddr!),
							let chatroom = core.searchChatRoom(
								params: nilParams, localAddr: nil, remoteAddr: peerAddr,
								participants: nil),
							chatroom.muted
						{
	...
	var body: some View {
		...
		.onOpenURL { url in
			if SharedMainViewModel.shared.displayedConversation != nil
				&& url.absoluteString.contains("linphone-message://")
			{
	...
	func selectedMainView() -> some View {
		ContentView()
			.onAppear {
				...
				{
					navigationManager.openChatRoom(
						callId: callId, peerAddr: peerAddr, localAddr: localAddr)
				}
			}
				...
				if let coreContext = self.coreContext {
					coreContext.doOnCoreQueue { core in
							...
							let identity = account.params?.identityAddress?.asStringUriOnly()
						{
							...
							if let pendingVoIPToken = UserDefaults.standard.string(
								forKey: "pendingVoIPToken")
							{
								Log.info(
									"BRD_FCM: Token VoIP pendente encontrado, enviando para API...")
								Task {
									await MiddlewareServices.registerAPNSToken(
										userID_Domain: identity, voipToken: pendingVoIPToken)
							...
							} else if let currentVoIPToken = UserDefaults.standard.string(
								forKey: "lastVoIPToken")
							{
								// Se não há pendente, verifica se precisa reenviar o atual
								Log.info(
									"BRD_FCM: Verificando se token VoIP atual precisa ser reenviado..."
								)
								Task {
									if MiddlewareServices.shouldUpdateToken(
										currentToken: currentVoIPToken)
									{
										Log.info(
											"BRD_FCM: Token VoIP precisa ser atualizado, enviando..."
										)
										await MiddlewareServices.registerAPNSToken(
											userID_Domain: identity, voipToken: currentVoIPToken)
									} else {
							...
							Log.warn(
								"BRD_FCM: Nenhuma conta disponível. Token será enviado após login.")
