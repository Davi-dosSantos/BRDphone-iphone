...
class CallInfo {
	...
	static func newOutgoingCallInfo(
		addr: Address, isSas: Bool, displayName: String, isVideo: Bool, isConference: Bool
	) -> CallInfo {
		let callInfo = CallInfo()
...
class ProviderDelegate: NSObject {
	...
	static var providerConfiguration: CXProviderConfiguration {
		...
		providerConfiguration.iconTemplateImageData = UIImage(named: "brdphone")?.pngData()
	...
	func reportIncomingCall(
		call: Call?, uuid: UUID, handle: String, hasVideo: Bool, displayName: String
	) {
		...
		Log.info(
			"CallKit: report new incoming call with call-id: [\(callId)] and UUID: [\(uuid.description)]"
		)
		...
		provider.reportNewIncomingCall(with: uuid, update: update) { error in
			if error == nil {
				Log.info(
					"[ProviderDelegate] provider.reportNewIncomingCall SUCCESS for UUID: \(uuid)")
				...
				Log.error(
					"[ProviderDelegate] provider.reportNewIncomingCall FAILED for UUID: \(uuid)")
				Log.error(
					"CallKit: cannot complete incoming call with call-id: [\(callId)] and UUID: [\(uuid.description)] from [\(handle)] caused by [\(error!.localizedDescription)]"
				)
		...
		DispatchQueue.main.asyncAfter(deadline: timeout) {
			CoreContext.shared.doOnCoreQueue(synchronous: true) { core in
				...
				if core.getCallByCallid(callId: callId ?? "") == nil {
					Log.info(
						"CallKit: terminate call with call-id: \(String(describing: callId)) and UUID: \(uuid) which does not exist."
					)
...
extension ProviderDelegate: CXProviderDelegate {
	func provider(_ provider: CXProvider, perform action: CXEndCallAction) {
		...
		CoreContext.shared.doOnCoreQueue { core in
			if let call = core.getCallByCallid(callId: callId ?? "") {
				...
				Log.info(
					"CallKit: Call ended with call-id: \(String(describing: callId)) an UUID: \(uuid.description)."
				)
			}
			action.fulfill()
		}
	...
	func provider(_ provider: CXProvider, perform action: CXAnswerCallAction) {
		let uuid = action.callUUID
		// Goal 4: Consultar o mapa thread-safe para obter o CallId real
		let callIdFromMap = TelecomManager.shared.getCallId(for: uuid)
		print("[CALLFLOW] ProviderDelegate.answerCallAction: UUID recebido: \(uuid)")
		print(
			"[CALLFLOW] Consultando callMap: CallId encontrado = \(String(describing: callIdFromMap))"
		)
		...
		CoreContext.shared.doOnCoreQueue { core in
			Log.info(
				"CallKit: answer call com CallId do mapa: \(String(describing: callIdFromMap)) e UUID: \(uuid.description)."
			)

			var call: Call? = nil
			if let realCallId = callIdFromMap {
				call = core.getCallByCallid(callId: realCallId)
			}
			...
			DispatchQueue.main.async {
				if UIApplication.shared.applicationState != .active {
					...
					if callLogIsNil {
						TelecomManager.shared.backgroundContextCameraIsEnabled =
							videoEnabledTmp == true || wasConferenceTmp == true
					} else {
						TelecomManager.shared.backgroundContextCameraIsEnabled =
							videoEnabledTmp == true
					}
			...
			if let callToAccept = call {
				TelecomManager.shared.acceptCall(
					core: core, call: callToAccept,
					hasVideo: callToAccept.params?.videoEnabled ?? false)
				print("[CALLFLOW] Chamada SIP encontrada e aceita!")
			} else {
				print("[CALLFLOW] Chamada SIP não encontrada para o UUID: \(uuid)")
			}
		...
		CoreContext.shared.doOnCoreQueue { core in
			...
			do {
				if call?.conference != nil && action.isOnHold {
					...
					Log.info(
						"CallKit: Call  with call-id: [\(callId)] and UUID: [\(uuid)] paused status changed to: [\(state)]"
					)
					if action.isOnHold {
						TelecomManager.shared.speakerBeforePause =
							AudioRouteUtils.isSpeakerAudioRouteCurrentlyUsed(core: core, call: call)
						try call!.pause()
						...
						if call != nil && call?.conference != nil && core.callsNb > 1 {
							...
							Log.info(
								"Assuming AudioSession is active when executing a CXSetHeldCallAction with isOnHold=false."
							)
				...
				Log.error(
					"CallKit: Call set held (paused or resumed) \(uuid) failed because \(error)")
				action.fail()
			}
		}
	}
	...
	func provider(_ provider: CXProvider, perform action: CXStartCallAction) {
		...
		if addr == nil {
			...
			CoreContext.shared.doOnCoreQueue { core in
				do {
					core.configureAudioSession()
					try TelecomManager.shared.doCall(
						core: core, addr: addr!, isSas: callInfo?.sasEnabled ?? false,
						isVideo: callInfo?.videoEnabled ?? false,
						isConference: callInfo?.isConference ?? false)
					action.fulfill()
				} catch {
	...
	func provider(_ provider: CXProvider, perform action: CXSetGroupCallAction) {
		CoreContext.shared.doOnCoreQueue { core in
			Log.info(
				"CallKit: Call grouped callUUid : \(action.callUUID) with callUUID: \(String(describing: action.callUUIDToGroupWith))."
			)
	...
	func provider(_ provider: CXProvider, perform action: CXSetMutedCallAction) {
		...
		CoreContext.shared.doOnCoreQueue { core in
			Log.info(
				"CallKit: Call muted with call-id: \(String(describing: callId)) an UUID: \(uuid.description)."
			)
	...
	func provider(_ provider: CXProvider, perform action: CXPlayDTMFCallAction) {
		...
		CoreContext.shared.doOnCoreQueue { core in
			Log.info(
				"CallKit: Call send dtmf with call-id: \(callId) an UUID: \(uuid.description).")
			if let call = core.getCallByCallid(callId: callId) {
	...
	func provider(_ provider: CXProvider, timedOutPerforming action: CXAction) {
		...
		let callId = callInfos[uuid]?.callId
		Log.error(
			"CallKit: Call time out with call-id: \(String(describing: callId)) an UUID: \(uuid.description)."
		)
		action.fulfill()
	}
