...
struct ContentView: View {
	...
	private var shouldShowCallView: Bool {
			...
			return telecomManager.callDisplayed
				&& (telecomManager.callInProgress || telecomManager.callConnected)
	...
	var body: some View {
		GeometryReader { geometry in
			VStack(spacing: 0) {
				let showAccountError =
					...
					&& (!telecomManager.callInProgress
						|| (telecomManager.callInProgress && !telecomManager.callDisplayed))
				if showAccountError {
					HStack {
						if let index = accountProfileViewModel.defaultAccountModelIndex,
							...
							coreContext.accounts[index].registrationStateAssociatedUIColor
								== .orangeWarning600
						{
				...
				if !sharedMainViewModel.fileUrlsToShare.isEmpty
					&& (!telecomManager.callInProgress
						|| (telecomManager.callInProgress && !telecomManager.callDisplayed))
				{
					HStack {
						...
						if sharedMainViewModel.fileUrlsToShare.count > 1 {
							Text(
								String(
									format: String(
										localized:
											"conversations_files_waiting_to_be_shared_multiple"),
									sharedMainViewModel.fileUrlsToShare.count.description)
							)
							.default_text_style_white(styleSize: 16)
						} else {
							Text(
								String(localized: "conversations_files_waiting_to_be_shared_single")
							)
							.default_text_style_white(styleSize: 16)
						}
				...
				if (telecomManager.callInProgress && !fullscreenVideo
					...
					|| isShowConversationFragment
				{
					HStack {
						...
						if callViewModel.callsCounter > 1 {
							Text(
								String(
									format: String(localized: "calls_count_label"),
									callViewModel.callsCounter.description)
							)
							.default_text_style_white(styleSize: 16)
						} else {
						...
						if callViewModel.callsCounter == 1 {
							Text(
								callViewModel.isPaused || telecomManager.isPausedByRemote
									? String(localized: "call_state_paused")
				...
				ZStack {
								...
								VStack(spacing: 0) {
									Group {
											...
											label: {
												VStack {
													Image("address-book")
														...
														.resizable()
														.foregroundStyle(
															sharedMainViewModel.indexView == 0
																? Color.orangeMain500
																: Color.grayMain2c600
														)
										...
										ZStack {
											...
											Button(
												action: {
													sharedMainViewModel.changeIndexView(
														indexViewInt: 1)
													sharedMainViewModel.displayedFriend = nil
													sharedMainViewModel.displayedConversation = nil
													sharedMainViewModel.displayedMeeting = nil
													if SharedMainViewModel.shared.missedCallsCount
														> 0
													{
														SharedMainViewModel.shared
												...
												label: {
													VStack {
														Image("phone")
															...
															.resizable()
															.foregroundStyle(
																sharedMainViewModel.indexView == 1
																	? Color.orangeMain500
																	: Color.grayMain2c600
															)
															.frame(width: 25, height: 25)
												...
												label: {
													VStack {
														Image("video-conference")
															...
															.resizable()
															.foregroundStyle(
																sharedMainViewModel.indexView == 3
																	? Color.orangeMain500
																	: Color.grayMain2c600
															)
															.frame(width: 25, height: 25)
									...
									VStack(spacing: 0) {
										if searchIsActive == false {
											HStack {
												...
												if let index = accountProfileViewModel
													.defaultAccountModelIndex,
													index < coreContext.accounts.count
												{

													let account = coreContext.accounts[index]
													let imagePath = account.getImagePath()
													let finalUrl = imagePath.appendingQueryItem(
														"v", value: UUID().uuidString)
													...
													.onReceive(
														NotificationCenter.default.publisher(
															for: NSNotification.Name("ImageChanged")
														)
													) { _ in
														imageTmp = nil
													}
												...
												.padding(
													.trailing,
													sharedMainViewModel.indexView == 2 ? 10 : 0)

												if sharedMainViewModel.indexView == 3 {
													Button {
														NotificationCenter.default.post(
															name: MeetingsListViewModel
																.ScrollToTodayNotification,
															object: nil)
													} label: {
														Image("calendar")
															.renderingMode(.template)
													...
													Menu {
														if sharedMainViewModel.indexView == 0 {
															Button {
																...
																HStack {
																	Text(
																		"contacts_list_filter_popup_see_all"
																	)
																	Spacer()
											...
											.roundedCorner(
												10, corners: [.bottomRight, .bottomLeft])
										} else {
											HStack {
												Button {
													withAnimation {
												...
												if #available(iOS 16.0, *) {
													...
													.onChange(of: text) { newValue in
															...
															sharedMainViewModel.indexView == 1
														{
															...
															sharedMainViewModel.indexView == 2
														{
															conversationsListVM.filterConversations(
																filter: text)
														} else if let meetingsListVM =
															meetingsListViewModel,
															sharedMainViewModel.indexView == 3
								...
								Group {
									...
									Button(
										action: {
											...
											sharedMainViewModel.displayedConversation = nil
											sharedMainViewModel.displayedMeeting = nil
											// BRD: Atualiza contatos apenas quando a aba é acessada
									...
									ZStack {
										if SharedMainViewModel.shared.missedCallsCount > 0 {
											VStack {
												HStack {
													Text(
														SharedMainViewModel.shared.missedCallsCount
															< 99
															? String(
																SharedMainViewModel.shared
																	.missedCallsCount)
					...
					if sharedMainViewModel.displayedFriend != nil
						|| sharedMainViewModel.displayedCall != nil
						|| sharedMainViewModel.displayedConversation != nil
						|| sharedMainViewModel.displayedMeeting != nil
					{
						HStack(spacing: 0) {
							...
							if let contactsListVM = contactsListViewModel,
								let displayedFriend = sharedMainViewModel.displayedFriend,
								sharedMainViewModel.indexView == 0
							{
								ContactFragment(
							...
							} else if let historyListVM = historyListViewModel,
								let displayedCall = sharedMainViewModel.displayedCall,
								sharedMainViewModel.indexView == 1
							{
								...
								let displayedConversation = sharedMainViewModel
									.displayedConversation, sharedMainViewModel.indexView == 2
							{
								...
								.background(Color.gray100)
								.ignoresSafeArea(.keyboard)
							} else if let meetingsListVM = meetingsListViewModel,
								let displayedMeeting = sharedMainViewModel.displayedMeeting,
								sharedMainViewModel.indexView == 3
							{
								MeetingFragment(
									isShowScheduleMeetingFragment: $isShowScheduleMeetingFragment,
							...
							title: Text(
								String(
									...
									contactsListVM.selectedFriend?.name
										?? (SharedMainViewModel.shared.displayedFriend!.name
											?? "Unknown Contact")
								)
							),
					...
					if sharedMainViewModel.operationInProgress {
						PopupLoadingView()
							...
							.onDisappear {
								if let contactsListVM = contactsListViewModel,
									let displayedConversation = contactsListVM.displayedConversation
								{
									...
									if !sharedMainViewModel.disableChatFeature {
										...
										if let conversationsListVM = self.conversationsListViewModel
										{
											...
											DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) {
												if let conversationsListVM = self
													.conversationsListViewModel
												{
													withAnimation {
														conversationsListVM.changeDisplayedChatRoom(
									...
									let displayedConversation = historyListVM.displayedConversation
								{
						...
						isShowSendCancelMeetingNotificationPopup
					{
						PopupView(
							...
							content: !sharedMainViewModel.disableChatFeature
								? Text("meeting_schedule_cancel_dialog_message") : Text(""),
							titleFirstButton: Text("dialog_cancel"),
							actionFirstButton: {
								sharedMainViewModel.displayedMeeting = nil
			...
			.onReceive(
				NotificationCenter.default.publisher(for: NSNotification.Name("ContactLoaded"))
			) { _ in
				if let conversationsListVM = conversationsListViewModel {
			...
			.onReceive(
				NotificationCenter.default.publisher(for: NSNotification.Name("ContactAdded"))
			...
			.onReceive(
				NotificationCenter.default.publisher(for: NSNotification.Name("CoreStarted"))
			) { _ in
				accountProfileViewModel.setAvatarModel()
			}
			.onReceive(
				NotificationCenter.default.publisher(
					for: NSNotification.Name("DefaultAccountChanged"))
			) { _ in
				accountProfileViewModel.defaultAccountModelIndex = CoreContext.shared.accounts
					.firstIndex(where: { $0.isDefaultAccount })
			...
			.onReceive(
				NotificationCenter.default.publisher(for: NSNotification.Name("PasswordUpdate"))
					.compactMap { $0.userInfo?["address"] as? String }
			) { address in
		...
		.onRotate { newOrientation in
			if (sharedMainViewModel.displayedFriend != nil
				|| sharedMainViewModel.displayedCall != nil
				|| sharedMainViewModel.displayedConversation != nil) && searchIsActive
			{
				self.focusedField = false
